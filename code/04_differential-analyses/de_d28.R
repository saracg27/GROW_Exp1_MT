### DE analysis for D28
### Run on the cluster using de_D28.sh from the code folder: sbatch de_D28.sh
...
library(DESeq2)
# Data
countData <- vroom(file = here::here('data', 'clean', 'merged_gene_abundance_D28.tsv'), col_names = TRUE)
meta <- read.csv(file = here::here('data', 'clean', 'metaD28.csv'), header = TRUE, sep = ",")
...
#histogram_raw <- hist(countData$`25-Mesocosm-1-1-Exp1-D28-Sed`, br = 100)

## Log2 transform and visualize
logCountData = log2(1 + countData)
#histogram_log <- hist(logCountData$`25-Mesocosm-1-1-Exp1-D28-Sed`, br = 50)
...
save.image(file = here("output", "DE_D28.RData"))
saveRDS(dds, file = here("output", "dds_D28.rds" ))
